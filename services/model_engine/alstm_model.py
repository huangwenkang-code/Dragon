"""
ALSTM (Attentive LSTM) model adapted from Microsoft Qlib.

ALSTM adds an attention mechanism on top of an RNN encoder:
1. A fully-connected + Tanh input projection
2. A configurable RNN backbone (GRU/LSTM)
3. An attention network that computes weighted sum of hidden states
4. The attention output is concatenated with the last hidden state
...
# =============================================================================
# ALSTM network module
# =============================================================================

class ALSTMNet(nn.Module):
    """Attentive LSTM network.
    ...
    hidden_size : int
        Hidden state dimension (used for both the RNN and attention).
    num_layers : int
        Number of recurrent layers.
    dropout : float
        Dropout applied between RNN layers and in the attention net.
    rnn_type : str
    ...
    def _build_model(self) -> None:
        ...
        # Output: concatenation of last hidden + attention output
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        inputs = inputs.permute(0, 2, 1)          # [batch, seq_len, input_size]
        rnn_out, _ = self.rnn(self.net(inputs))    # [batch, seq_len, hidden_size]
        attention_score = self.att_net(rnn_out)    # [batch, seq_len, 1]
        out_att = torch.mul(rnn_out, attention_score)
        out_att = torch.sum(out_att, dim=1)        # [batch, hidden_size]
        out = self.fc_out(
...
class ALSTMModel(BaseModel):
    """Attentive LSTM training wrapper for time-series regression.
    ...
    hidden_size : int
        Hidden dimension for RNN and attention.
    num_layers : int
        Number of recurrent layers.
    dropout : float
        Dropout rate.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._model = ALSTMNet(d_feat, hidden_size, num_layers, dropout, rnn_type)
        logger.info("ALSTM model size: %.4f MB", count_parameters(self._model))
    ...
    def predict(self, X: ArrayLike) -> np.ndarray:
        ...
        if not self._fitted:
            raise RuntimeError("ALSTMModel is not fitted yet. Call fit() first.")
