"""
GRU model adapted from Microsoft Qlib.

Gated Recurrent Unit encoder for financial time-series forecasting.
Same architecture as the LSTM variant but using a GRU backbone.
...
class GRUNet(nn.Module):
    """Multi-layer GRU followed by a linear output head.
    ...
    dropout : float
        Dropout applied between GRU layers (ignored if num_layers == 1).
    """
...
class GRUModel(BaseModel):
    """GRU training wrapper for time-series regression.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._model = GRUNet(d_feat, hidden_size, num_layers, dropout)
        logger.info("GRU model size: %.4f MB", count_parameters(self._model))
    ...
    def predict(self, X: ArrayLike) -> np.ndarray:
        ...
        if not self._fitted:
            raise RuntimeError("GRUModel is not fitted yet. Call fit() first.")
